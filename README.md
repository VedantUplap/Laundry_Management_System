...
### 1. Why cannot a browser JavaScript directly connect to MySQL using JDBC?
**Answer:** JDBC is a native Java API that establishes low-level TCP/IP socket connections (port 3306). Web browsers execute sandboxed JavaScript that can only send standard HTTP/HTTPS or WebSocket network requests. Directly exposing MySQL to client-side JavaScript would also compromise database credentials and security. The Java HTTP Server acts as the secure middle layer.

### 2. Explain the End-to-End Request Flow.
```
1. Browser UI makes Fetch call (e.g. POST /api/orders).
2. Java HttpServer receives HTTP request on port 3001.
3. ApiDispatcher validates HMAC-SHA256 JWT & checks RBAC role permissions.
4. OrderDAO obtains JDBC Connection from DBConnection.
5. PreparedStatement compiles parameterized SQL query.
6. Multi-table transaction executes (ORDERS -> ORDER_DETAILS -> DELIVERY).
7. MySQL triggers automatically calculate totals.
8. conn.commit() commits transaction atomically.
9. ResultSet is extracted into Java Map / DTO.
10. Gson serializes response to JSON and returns HTTP 201 to the browser.
```

### 3. How does JDBC protect against SQL Injection?
**Answer:** By using `java.sql.PreparedStatement`. The SQL command is pre-compiled by the database engine with `?` parameter markers. Input values are sent separately and treated strictly as literal data, preventing any malicious input from modifying the SQL query structure.

### 4. How are Transactions managed in JDBC?
**Answer:**
```java
conn.setAutoCommit(false); // Begin transaction
try {
    // Execute related SQL operations
...
This project is developed for academic presentation and demonstration in the **Database Management Systems (DBMS)** course. All rights reserved.
